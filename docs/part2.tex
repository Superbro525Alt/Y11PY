...
\begin{lstlisting}[caption={Handling packets and broadcasting}],label={lst:server_process}
    ...
    def _broadcast_loop(self):
        while True:
            self.tick()
            for h in self.handlers: h.tick()
            sleep(0.1)
\end{lstlisting}
The server dispatches to \texttt{NetworkStateObject} handlers for login, sync, matchmaking and deploy 
...
\end{lstlisting}
Concrete \texttt{GameNetworkClient} drives LOGIN, SYNC, MATCH\_REQUEST and DEPLOY\_UNIT interactions from the UI .
...
\begin{itemize}[nosep]
  \item \textbf{Cycle Time:} Average time from issue creation to merge was 2.1 days.  
  \item \textbf{Deployment Frequency:} Deployed to staging environment twice weekly after passing CI and smoke tests.  
  \item \textbf{Defect Rate:} Post‐release critical bugs averaged 0.5 per release, down from 1.8 in initial sprints.  
  \item \textbf{Code Quality:}  
    \begin{itemize}[nosep]
      \item Code coverage consistently $\ge$ 95\%.  
      \item Mutation testing score stabilized at 85\%, indicating robust test suite.
    \end{itemize}
\end{itemize}
