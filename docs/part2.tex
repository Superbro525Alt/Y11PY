...
\subsection*{Participant Satisfaction Scores}
\begin{table}[h]
\centering
\begin{tabular}{lcccc}
\toprule
\textbf{Participant} & \textbf{Usability} & \textbf{Doc Clarity} & \textbf{Load Resp.} & \textbf{Tutorial} \\
\midrule
A & 4.7 & 4.8 & 4.3 & 4.5 \\
B & 5.0 & 4.9 & 4.6 & 4.8 \\
C & 4.6 & 5.0 & 4.4 & 4.6 \\
...
E & 4.8 & 4.9 & 4.5 & 4.8 \\
\midrule
\textbf{Average} & \textbf{4.8} & \textbf{4.9} & \textbf{4.5} & \textbf{4.6} \\
\bottomrule
\end{tabular}
\caption{Individual and average satisfaction scores (out of 5).}
\end{table}

\subsection*{Average Satisfaction Across Metrics}
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xlabel={Metric},
      ylabel={Score (out of 5)},
      ymin=0, ymax=5.5,
      symbolic x coords={Usability,Documentation,Responsiveness,Tutorial},
      xtick=data,
      nodes near coords,
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Average user satisfaction scores by metric.}
\end{figure}

\subsection*{Additional Observations}
\begin{itemize}[nosep]
  \item \textbf{Tutorial Completion Time:}  
    Mean time 3.2 min (SD 0.4 min).
  \item \textbf{Load Times (three standard battles):}  
    Mean 1.4 s (SD 0.2 s).
  \item \textbf{Error Encounters:}  
    Only one minor visual glitch reported (1/15 runs).
\end{itemize}

\subsection*{Suggestions (by frequency)}
\begin{itemize}[nosep]
  \item Path overlay (5/5 users)  
  \item Quick‐save feature (3/5 users)  
  \item Contextual tooltips (2/5 users)  
\end{itemize}
