...
def card_tick(_card: IDUnit, arena: Arena) -> Optional[IDUnit]:
    ...
    if card.inner.unit_data.current_target and (path := card.inner.unit_data.current_target.path) and len(path) > 1 and card.inner.underlying.card_type == CardType.TROOP.value and datetime.now().timestamp() - (datetime.strptime(card.inner.unit_data.last_move, DATE_FORMAT) + timedelta(seconds=MovementSpeed.to_num(card.inner.underlying.movement_speed))).timestamp() > 0:
