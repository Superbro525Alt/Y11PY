...
from network import Client, NetworkObject, Packet, Server, ServerStatus, deserialize_object, do_if
...
from engine import Camera, Camera3D, Component, EngineCode, EngineFrameData, GameObject, TextRenderer, Transform, Transform3D, UIAnimation, UIElement
...
#
    def tick_state(self, data: bytes) -> None:
        with self.state_lock:
            try:
                self.state = json.loads(data.decode(), object_hook=lambda d: SimpleNamespace(**d))
            except Exception as e:
                # print("e")
                raise ValueError(f"Transmitted Game State was in an invalid format: {e}")
    ...
    def start_matchmaking(self) -> bool:
        ...
        self.send(Packet.from_struct(PacketType.MATCH_REQUEST, MatchRequest(self.state.menu_state.trophies, self.auth_state.uuid)))
...
class NetworkStateObject(NetworkObject):
    def __init__(self):
        super().__init__()
        self._handles = [PacketType.LOGIN, PacketType.CLIENT_SERVER_SYNC, PacketType.MATCH_REQUEST]
        self.users: UserMap = UserMap([User(str(i), str(i), str(i), ServerUserData([generate_chest(ChestRarity.GOLD)], None, 0, [Deck([ARCHER, GIANT])], 0, None)) for i in range(2)])
        self.shop = Shop([])
        ...
        if packet.packet_type == PacketType.LOGIN:
            ...
            if found is not None:
                ...
                if found.data.current_battle:
                    found_match, other_uuid = self.matchmaking.get_match(found.data.current_battle, data.uuid)
                    if found_match and other_uuid:
                        state.battle_state = BattleState(found_match.elixir, found_match.hand, found_match.next_card, found.data.current_battle, other_uuid)
...
class Game(Engine):
    ...
    def setup_ui(self):
        title_banner = UIElement(int(self.sdl.get_width() / 6), 50, int(self.sdl.get_width() - self.sdl.get_width() / 6 * 2), 100, color=(255, 215, 0))  

        self.ui_manager.add_element(title_banner)
        
        bottom_bar = UIElement(0, self.sdl.get_height() - 120, self.sdl.get_width(), 120, color=(40, 40, 60))
        ...
        for i, label in enumerate(button_labels):
            button = UIElement(start_x + i * (button_width + button_spacing), 
                               self.sdl.get_height() - 100, 
                               button_width, 80, color=(200, 200, 255), on_hover=(255, 0, 0))
